...
-- Date: 2026-02-17
-- Purpose: Keep fixed 14-day window, favor more crew, and remove training-count bias in scoring.
  ...
  while v_day <= v_end_date loop
    ...
    scored_crew as (
      select
        group_id,
        crew_id,
        max(public.urgency_score(v_day, due_date, days_overdue, days_until_due, status_label)) as crew_u,
        max(case when coalesce(days_overdue, 0) >= 30 then 1 else 0 end) as crew_is_extreme
      from need_rows
      group by group_id, crew_id
    ),
    group_rollup as (
      select
        g.id as group_id,
        count(sc.crew_id) as people_helped,
        sum(sc.crew_is_extreme) as extreme_count,
        max(sc.crew_u) as base_u,
        count(sc.crew_id) as need_count
      from public.training_groups g
      left join scored_crew sc
        on sc.group_id = g.id
      where g.location_id = p_location_id
        and g.active = true
      group by g.id
      having count(sc.crew_id) > 0
    ),
    group_bonus as (
      select
        sc.group_id,
        sum(sc.crew_u) - max(sc.crew_u) as sum_others_u
      from scored_crew sc
    ...
    group_scored as (
      select
        ...
        r.extreme_count,
        r.base_u,
        (0.20 * coalesce(b.sum_others_u, 0)) as bonus_u,
        ((8 * ln(1 + r.people_helped::numeric)) + (6 * r.people_helped::numeric)) as eff_u,
        (
          r.base_u
          + (0.20 * coalesce(b.sum_others_u, 0))
          + ((8 * ln(1 + r.people_helped::numeric)) + (6 * r.people_helped::numeric))
        ) as total_score
      from group_rollup r
