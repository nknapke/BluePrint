-- Crew day-hours patch: PTO pays 7.5 regular hours
-- Date: 2026-03-02
-- Purpose:
--   If a day is marked PTO, grant 7.5 paid regular hours for that day.
--
-- Assumption:
--   PTO is paid, but it does not create daily OT and does not count toward
--   the 40-hour weekly OT threshold. It is added after OT classification.
--
-- Existing MIT rules remain:
--   - paid lunch
--   - no daily OT
--   - weekly OT only after 40 paid worked hours
...
pto_days as (
  select distinct
    ...
    s.crew_id
  from public.crew_work_shifts s
  where upper(btrim(coalesce(s.day_description, ''))) = 'PTO'
),
valid_shifts as (
  select
    sb.location_id,
    ...
    sb.dept_key,
    coalesce(sb.dept_key = 'mit', false) as is_mit_department,
    sb.start_minute,
    sb.end_minute,
    case
...
minute_rollup as (
  select
    ...
    cm.work_date,
    cm.crew_id,
    bool_or(cm.is_mit_department) as is_mit_department,
...
lunch_plan as (
  select
    mr.*,
    case
      when mr.is_mit_department then 0
...
daily_capacity as (
  select
    dp.*,
    case
      when dp.is_mit_department then dp.paid_day_minutes
      else least(dp.paid_day_minutes, 480)
    end::int as daily_base_minutes
  from day_paid dp
),
daily_split as (
  select
    dc.*,
    least(dc.regular_paid_minutes, dc.daily_base_minutes)::int as regular_daily_base_minutes,
    least(
      dc.lead_paid_minutes,
      greatest(
        0,
        dc.daily_base_minutes
        - least(dc.regular_paid_minutes, dc.daily_base_minutes)
      )
...
select
  out_rows.location_id,
  out_rows.work_date,
  out_rows.crew_id,
  out_rows.total_hours,
  out_rows.lead_hours,
  out_rows.hours,
  out_rows.regular_overtime_hours,
  out_rows.lead_overtime_hours
from (
  select
    fs.location_id,
    fs.work_date,
  ...
  select
    ...
    p.work_date,
    p.crew_id,
    7.50::numeric(10,2) as total_hours,
    0::numeric(10,2) as lead_hours,
    7.50::numeric(10,2) as hours,
    0::numeric(10,2) as regular_overtime_hours,
    0::numeric(10,2) as lead_overtime_hours
  from pto_days p
  where not exists (
    select 1
    from final_split fs
    where fs.location_id = p.location_id
      and fs.work_date = p.work_date
      and fs.crew_id = p.crew_id
  )
) out_rows;
